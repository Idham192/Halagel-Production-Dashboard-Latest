...
import { Trash2, Download, Calendar, List, Filter, XCircle } from 'lucide-react';
import { getTodayISO } from '../../utils/dateUtils';

export const ProductionLog: React.FC = () => {
  const { hasPermission } = useAuth();
  const [data, setData] = useState<ProductionEntry[]>(StorageService.getProductionData());
  ...
  const downloadCSV = () => {
    ...
    if (viewMode === 'daily') {
        headers = ["Date", "Category", "Process", "Product", "Plan", "Actual", "Efficiency %", "Batch No", "Manpower"];
        rows = filteredData.map(d => [
            d.date, d.category, d.process, `"${d.productName}"`, d.planQuantity, d.actualQuantity, 
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-slate-700 flex flex-wrap gap-4 items-end">
        <div className="space-y-1">
          ...
          <input 
            ...
            className="px-3 py-2 text-sm bg-gray-50 dark:bg-slate-900 rounded-lg border dark:border-slate-700 outline-none focus:ring-2 focus:ring-indigo-500 w-full sm:w-auto"
          />
        </div>
        <div className="space-y-1">
          <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest flex items-center gap-1">
        ...
        <div className="space-y-1">
          ...
          <select 
            ...
            className="px-3 py-2 text-sm bg-gray-50 dark:bg-slate-900 rounded-lg border dark:border-slate-700 outline-none focus:ring-2 focus:ring-indigo-500 min-w-[120px]"
          >
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-gray-50 dark:bg-slate-700 text-gray-700 dark:text-slate-200 font-semibold">
              {viewMode === 'daily' ? (
                  <tr>
                  ...
                  filteredData.length === 0 ? (
                    ...
                    return (
                      <tr key={entry.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                        <td className="px-6 py-3 font-mono text-xs">{entry.date}</td>
                        <td className="px-6 py-3">
                          <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-900 text-slate-500">
                            {entry.category}
                          </span>
                        </td>
                        <td className="px-6 py-3">
                          <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600">
                            {entry.process}
                          </span>
                        </td>
                        <td className="px-6 py-3 font-bold text-slate-700 dark:text-slate-200">{entry.productName}</td>
                        <td className="px-6 py-3 text-right font-mono text-blue-600">{entry.planQuantity.toLocaleString()}</td>
                        <td className="px-6 py-3 text-right font-mono text-emerald-600">{entry.actualQuantity.toLocaleString()}</td>
                        <td className="px-6 py-3 text-right">
                            <span className={`px-2 py-0.5 rounded text-xs font-bold ${eff >= 100 ? 'text-emerald-500' : eff >= 80 ? 'text-blue-500' : 'text-red-500'}`}>
                                {eff}%
                            </span>
                        </td>
                        {hasPermission(['admin', 'manager']) && (
                            <td className="px-6 py-3 text-center">
                                <button onClick={() => handleDelete(entry.id)} className="text-gray-400 hover:text-red-500 p-2 rounded-full hover:bg-rose-50 dark:hover:bg-rose-900/20 transition">
